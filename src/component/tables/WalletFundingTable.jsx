...
export default function WalletFundingTable({ data = [] }) {
					...
					{paginatedData.length === 0 ? (
						...
						paginatedData.map((item, index) => (
							<TableRow key={item.id} className={`${index % 2 === 0 ? "bg-white" : "bg-stone-100"}`}>
								...
								<TableCell className="text-blue-600 font-medium cursor-pointer">{item.reference}</TableCell>
								<TableCell>{item.send_amount}</TableCell>
								<TableCell>{item.receive_amount}</TableCell>
								<TableCell>{item.pay_in_method}</TableCell>
